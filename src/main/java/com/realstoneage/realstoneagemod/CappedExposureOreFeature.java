...
// A copy of vanilla's OreFeature, modified so that at most one block per vein is
// allowed to place when exposed to air (every other exposed candidate in the same
// vein is discarded instead), and the vein as a whole never places more than
// MAX_TOTAL_PLACED blocks - so finding the exposed block doesn't lead into a full
// vanilla-size buried vein behind it. config.size is left alone and still drives the
// vein's geometric spread/radius (same odds of a vein reaching an existing cave or
// cliff face as vanilla), only the number of blocks actually placed is capped.
// Used for the above-Y48 "surface" iron/copper ore variants - see the
// realstoneage:*_surface configured features.
public class CappedExposureOreFeature extends Feature<OreConfiguration> {
    private static final int MAX_TOTAL_PLACED = 2;

    public CappedExposureOreFeature(Codec<OreConfiguration> codec) {
        super(codec);
    }
                                            ...
                                            if (!tested.get(bitSetIndex)) {
                                                ...
                                                if (level.ensureCanWrite(orePos)) {
                                                    ...
                                                    if (section != null) {
                                                        ...
                                                        boolean exposed = Feature.isAdjacentToAir(sectionGetter::getBlockState, orePos);
                                                        if (placed < MAX_TOTAL_PLACED && (!exposed || exposedPlaced < 1)) {
